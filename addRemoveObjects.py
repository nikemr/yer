...
class Yer(DirectObject):
    ...
    def processInput(self, dt):
        ...
        if inputState.isSet('removeNode'):            
            if self.t:
                self.t=0
                print(type(self.worldNP.find("m*").node()))
                print(self.worldNP.find("m*").node())
                print(self.world)
                self.world.remove(self.worldNP.find("m*").node())
                self.worldNP.find("m*").detachNode()                
                print(f'dolu mu:   {self.worldNP.find("m*")}')
    ...
    def agent_maker(self):
        ...
        #nodepath---------------------------
        agent_node_path = self.worldNP.attachNewNode(BulletRigidBodyNode('maked_agent'))
